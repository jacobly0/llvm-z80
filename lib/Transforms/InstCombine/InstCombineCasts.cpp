...
Instruction *InstCombiner::visitTrunc(TruncInst &CI) {
  ...
  if (Src->hasOneUse() &&
      match(Src, m_Shr(m_ZExt(m_Value(A)), m_ConstantInt(Cst)))) {
    ...
    // Since we're doing a zero extend, the sign bit must be zero, and we know
    // that the shift amount is smaller than ASize, so it is always safe to do
    // a lshr in A's type, then zero extend or truncate to the result.
